...
describe("passtZu", () => {
  ...
  it("prüft ein Merkmal nur gegen das Feld seiner Art", () => {
    ...
    expect(passtZu(alsId, spur({ gegenpartei: "Netcup GmbH" }))).toBe(false);
  ...
  it("versteht * als beliebigen Text", () => {
    ...
    expect(passtZu(e, spur({ gegenpartei: "Stadtwerke Bonn" }))).toBe(true);
  ...
  it("nimmt alles außer dem Stern wörtlich", () => {
    ...
    expect(passtZu(e, spur({ gegenpartei: "E.ON Energie Deutschland" }))).toBe(true);
  ...
  it("trifft sowohl den Namen aus dem Auszug als auch seine normalisierte Form", () => {
    const roh: Vertragserkennung = {
      vertragId: "v1",
      merkmale: [{ art: "empfaenger", muster: "Netcup GmbH" }],
    };
    expect(passtZu(roh, spur({ gegenpartei: "Netcup GmbH" }))).toBe(true);
