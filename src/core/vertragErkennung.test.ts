...
describe("anbieterSchluessel", () => {
  it("fasst dieselbe Firma trotz Rechtsform und Schreibweise zusammen", () => {
    expect(anbieterSchluessel("Netcup GmbH")).toBe(anbieterSchluessel("netcup"));
   ...
   * Stadtwerke") stiftet mehr Schaden als zwei getrennte, denn er nennt einen Betrag,
  ...
  it("wirft verschiedene Anbieter mit gleichem Anfang NICHT zusammen", () => {
    expect(anbieterSchluessel("Stadtwerke Bonn")).not.toBe(anbieterSchluessel("Stadtwerke Bremen"));
  });
});

describe("vertragskandidaten", () => {
  it("erkennt eine monatliche Zahlung mit Rhythmus und Median-Betrag", () => {
    const k = vertragskandidaten(reihe({ name: "Netcup", betrag: 1650, n: 12, tage: 30 }), HEUTE);
    expect(k).toHaveLength(1);
    expect(k[0].anbieter).toBe("Netcup");
  ...
  it("merkt sich Konto und Kategorie der Zahlungen für die Vorbelegung", () => {
    const spuren = reihe({ name: "Netcup", betrag: 1650, n: 12, tage: 30 }).map((s, i) => ({
      ...s,
  ...
  it("gruppiert über die Gläubiger-ID, auch wenn der Name im Auszug wechselt", () => {
    const spuren = [
      ...reihe({ name: "O2", betrag: 3848, n: 6, tage: 30, glaeubigerId: "DE9700000000142462" }),
      ...reihe({ name: "Telefonica Germany GmbH", betrag: 3848, n: 6, tage: 30, bis: "2024-08-16", glaeubigerId: "DE9700000000142462" }),
    ];
    const k = vertragskandidaten(spuren, HEUTE);
    expect(k).toHaveLength(1);
    expect(k[0].anzahl).toBe(12);
    expect(k[0].glaeubigerId).toBe("DE9700000000142462");
  });
  ...
  it("hält einen Vertrag für laufend, dessen letzte Zahlung einen Monat her ist", () => {
    const k = vertragskandidaten(reihe({ name: "Targobank", betrag: 28730, n: 12, tage: 30, bis: "2026-07-15" }), HEUTE);
    expect(k).toHaveLength(1);
    expect(k[0].laeuft).toBe(true);
  });
...
describe("vertragZuGegenpartei", () => {
  const vertraege = [{ anbieter: "Telefonica Germany GmbH" }, { anbieter: "Stadtwerke Bonn" }];
  ...
  it("verwechselt Anbieter mit gleichem Anfang nicht", () => {
    expect(vertragZuGegenpartei(vertraege, "Stadtwerke Bremen")).toBeUndefined();
  });
