...
describe("Trifft", () => {
  it("trifft den rohen Namen", () => {
    expect(festlegungTrifft(f("Netflix International BV", "k1"), "netflix international bv")).toBe(true);
  });
  ...
  it("nimmt den Stern als beliebigen Text", () => {
    ...
    expect(festlegungTrifft(f("dm*", "k1"), "Rossmann")).toBe(false);
  });
...
describe("Welche Festlegung gilt", () => {
  it("liefert null, wenn keine passt", () => {
    expect(festlegungFuer([f("rewe", "k1")], "Netflix")).toBeNull();
  });
