...
import { merkmaleFuer, merkmalsbefund, namensraum, STOPPWOERTER } from "./merkmale";
...
describe("Empfänger", () => {
  ...
  it("ein einwortiger Name bekommt KEIN Teil-Token", () => {
    ...
    const m = merkmaleFuer(quelle({ gegenpartei: "Netcup GmbH" }));
...
describe("Verwendungszweck", () => {
  ...
  it("wirft Referenznummern und Ziffernfolgen weg", () => {
    const b = merkmalsbefund(
      quelle({ verwendungszweck: "Rechnung RE2026004711 vom 15.03.2026 IBAN DE89370400440532013000" }),
    );
    expect(b.merkmale.some((m) => m.includes("2026004711"))).toBe(false);
    expect(b.merkmale.some((m) => m.includes("de89370400440532013000"))).toBe(false);
  ...
  it("schneidet angeklebte Nummern ab, statt das Wort wegzuwerfen", () => {
    ...
    const m = merkmaleFuer(quelle({ verwendungszweck: "DEBITKARTE2025 3386COMDIRECT" }));
  ...
  it("wirft weg, wenn nach dem Abschneiden nur ein Präfix übrig bleibt", () => {
    ...
    const b = merkmalsbefund(quelle({ verwendungszweck: "DE89370400440532013000 RE2026004711" }));
  ...
  it("erkennt maskierte Kartennummern als Platzhalter", () => {
    ...
    expect(b.verworfen).toContainEqual({ wort: "xxxx", grund: "platzhalter" });
  ...
  it("die Statistik nennt das Originalwort, nicht den gekürzten Kern", () => {
    ...
    expect(b.verworfen).toContainEqual({ wort: "re2026004711", grund: "ziffern" });
  });

  it("wirft Bank-Boilerplate als Stoppwort weg", () => {
    ...
    expect(b.verworfen.every((v) => v.grund === "stoppwort")).toBe(true);
  });
