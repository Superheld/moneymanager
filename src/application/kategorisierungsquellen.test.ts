...
const ZAHLUNG = {
  ...
  betrag: -999,
  gegenpartei: "Netflix International BV",
...
describe("Quellen laden", () => {
  ...
  it("nimmt Verträge mit Kategorie samt ihrer Regeln auf", async () => {
    const q = await kategorisierungsquellen(
      repos({
        vertraege: [{ id: "v1", anbieter: "Netflix", beginn: "2026-01-01", verlaengerung: "keine", status: "aktiv", kategorieId: "k-abo" }],
  ...
  it("lässt Regeln weg, wenn kein Vertrag eine Kategorie trägt", async () => {
    ...
    const q = await kategorisierungsquellen(
      repos({
        vertraege: [{ id: "v1", anbieter: "Netflix", beginn: "2026-01-01", verlaengerung: "keine", status: "aktiv" }],
        erkennungen: [standardErkennung("v1", "Netflix International", 999)],
      }),
    );
