...
function bank(over: Partial<RohUmsatz> = {}): RohUmsatz {
  return {
    ...
    betrag: -4990,
    waehrung: "EUR",
    gegenpartei: "EDK*EDEKA MUSTERMARKT",
    verwendungszweck: "KARTENVERFÜGUNGEDK*EDEKA MUSTERMARKT, MUSTERSTADT  DE",
...
function datei(over: Partial<RohUmsatz> = {}): RohUmsatz {
  return {
    ...
    waehrung: "EUR",
    gegenpartei: "Edeka",
    verwendungszweck: "EDK*EDEKA MUSTERMARKT, MUSTERSTADT DEKarte Nr. 1234 56XX XXXX 7890",
    kontoIban: "DE02120300000000202051",
    istUmbuchung: false,
    quelle: "finanzguru",
    nativeId: "fg-1",
...
describe("zweckKern", () => {
  it("schneidet den Buchungstext ab, den nur die Bank mitliefert", () => {
    ...
    expect(zweckKern("KARTENVERFÜGUNGEDEKA, MUSTERSTADT")).toBe("edekamusterstadt");
    expect(zweckKern("Edeka, Musterstadt")).toBe("edekamusterstadt");
  });
...
describe("vergleiche", () => {
  ...
  it("erkennt dieselbe Lastschrift über Gläubiger-ID und Mandatsreferenz", () => {
    const a = bank({ glaeubigerId: "DE17ZZZ00000001797", mandatsreferenz: "M-4711", verwendungszweck: "" });
    const b = datei({ glaeubigerId: "DE17ZZZ00000001797", mandatsreferenz: "M-4711", nativeId: undefined, verwendungszweck: "" });
  ...
  it("trennt Konten, auch wenn alles andere passt", () => {
    const b = vergleiche(bank(), datei({ nativeId: undefined, kontoIban: "DE02500105170137075030" }));
    expect(b.urteil).toBe("verschieden");
  });
