...
describe("Vertrag", () => {
  /** Ein Vertrag über 9,99 € bei Netflix, mit Kategorie. */
  function mitVertrag(): Vorschlagskontext {
    return {
      ...nurKatalog,
      erkennungen: [standardErkennung("v1", "Netflix International", 999)],
      vertragsKategorie: new Map([["v1", "k-abo"]]),
    };
  }

  it("erbt die Kategorie des Vertrags, den die Zahlung trifft", () => {
    const v = vorschlagFuer(roh({ gegenpartei: "Netflix International BV", betrag: -999 }), mitVertrag());
    expect(v).toEqual({ kategorieId: "k-abo", charakter: "Aufwand", quelle: "regel" });
  });

  it("nennt den Vertrag im Befund", () => {
    const b = vorschlagsbefundFuer(roh({ gegenpartei: "Netflix International BV", betrag: -999 }), mitVertrag());
  ...
  it("greift nicht bei einer Zahlung außerhalb der Betragsspanne", () => {
    ...
    const v = vorschlagFuer(roh({ gegenpartei: "Netflix International BV", betrag: -9900 }), mitVertrag());
  ...
  it("ein Vertrag OHNE Kategorie schlägt nichts vor", () => {
    ...
    expect(vorschlagFuer(roh({ gegenpartei: "Netflix International BV", betrag: -999 }), kontext)).toBeUndefined();
  });
  ...
  it("schlägt den Vertrag VOR das Remapping", () => {
    ...
    const v = vorschlagFuer(
      roh({ gegenpartei: "Netflix International BV", betrag: -999, kategorieHinweis: "Lebensmittel" }),
      mitVertrag(),
    );
  ...
  it("die Umbuchung schlägt auch den Vertrag", () => {
    const v = vorschlagFuer(
      roh({ gegenpartei: "Netflix International BV", betrag: -999, istUmbuchung: true }),
      mitVertrag(),
    );
    expect(v).toEqual({ charakter: "Umschichtung", quelle: "umbuchung" });
  });
...
describe("Festlegung", () => {
  /** „Zahlungen an Netflix sind immer Abos & Streaming." */
  ...
  it("kennt keine Betragsspanne — eine Kategorie ist eine Klasse, kein Vertrag", () => {
    ...
    const v = vorschlagFuer(roh({ gegenpartei: "Netflix International BV", betrag: -190_00 }), mitFestlegung());
    expect(v?.kategorieId).toBe("k-abo");
  });
  ...
  it("schlägt den Vertrag", () => {
    ...
    expect(vorschlagFuer(roh({ gegenpartei: "Netflix International BV", betrag: -999 }), kontext)?.kategorieId).toBe("k-abo");
  });
  ...
  it("die Umbuchung schlägt auch die Festlegung", () => {
    ...
    const v = vorschlagFuer(roh({ gegenpartei: "Netflix International BV", istUmbuchung: true }), mitFestlegung());
    expect(v).toEqual({ charakter: "Umschichtung", quelle: "umbuchung" });
  });
