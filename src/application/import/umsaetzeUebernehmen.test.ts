...
describe("Dublettenfinder beim Übernehmen", () => {
    ...
    await umsaetzeUebernehmen(
      {
        ...
        konten: [{ quelleKey: "DE02120300000000202051", neu: { bezeichnung: "Giro", typ: "Giro" } }],
      },
      f.deps,
    );
    return f;
  ...
  it("legt eine wiedererkannte Buchung nicht nochmal an, sondern ergänzt sie", async () => {
    ...
    const ergebnis = await umsaetzeUebernehmen(
      {
        ...
        konten: [{ quelleKey: "DE02120300000000202051", kontoId: f.konten[0].id }],
      },
      f.deps,
    );
    ...
    expect(f.umsaetze[0].gegenpartei).toBe("Edeka");
  });
