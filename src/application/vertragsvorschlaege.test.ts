...
describe("vertragsvorschlaege", () => {
  it("verbindet Buchung und Umsatz über istbuchungId", async () => {
    const { buchungen, umsaetze } = monatsreihe("a", "Netcup GmbH", 1650, { glaeubigerId: "DE49NET00000504360" });
    ...
    expect(k[0].anbieter).toBe("Netcup GmbH");
    expect(k[0].glaeubigerId).toBe("DE49NET00000504360");
  ...
  it("findet nichts, wenn zu den Buchungen keine Umsätze gehören", async () => {
    const { buchungen } = monatsreihe("a", "Netcup", 1650);
  ...
  it("blendet aus, was schon als Vertrag erfasst ist", async () => {
    const a = monatsreihe("a", "Netcup GmbH", 1650);
